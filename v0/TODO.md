...
**Files:** `alsc/references/module-schema-definition.md`, `alsc/references/frontmatter-schema-definition.md`, `alsc/references/record-validation.md`, `alsc/references/diagnostic-codes.md`
...
**Files:** `alsc/references/record-validation.md:133-135`, `example-systems/pristine-happy-path/workspace/experiments/MODULE.md:8-10`
...
2. **Who creates/updates workspace `.schema/`?** `als-migrate` Step 5 updates `MODULE.md` and the skill router but never mentions updating `workspace/.schema/`. Is it a build artifact generated by the compiler? Manually maintained? Copied during migration?
...
**Files:** `alsc/references/record-validation.md:19`, `alsc/references/module-skill-definition.md:79`

---

### 5. CLI interface for `alsc validate` is undesigned

**Status:** unresolved
**Severity:** significant — blocks compiler packaging and integration with als-migrate

`alsc validate` is referenced in `als-migrate` (Steps 2, 4, 5) with `<!-- TODO -->` markers. The compiler output shape is fully defined (`alsc/references/compiler-error-shape.md`), but the invocation interface is not:

1. **Arguments:** `als-migrate` uses two different invocation forms:
   - `alsc validate <module_skill_path>` (Step 2)
   - `alsc validate <module_data_path> --schema <path>` (Steps 4, 5)
...
4. **Scope:** Does `alsc validate` validate one module or all modules in a workspace? Can it validate a single file?
...
**Files:** `alsc/skills/als-migrate/SKILL.md:34-36,74-75,101-102`
...
Cross-module reference validation (record-validation Phase 6) requires the compiler to resolve URIs like `als://workspace/people/person/PPL-000101`. This means it must locate the `people` module, load its records, and build a canonical identity index.
...
2. **No `.als` marker or config file.** Nothing marks a directory as an ALS workspace root.
...
**Files:** `alsc/references/record-validation.md:46-48`, `alsc/references/module-schema-definition.md:21-23`
...
**Files:** `alsc/references/record-validation.md:97-98`, `alsc/references/content-schema-definition.md:17`
...
**Files:** `alsc/references/record-validation.md:127-128`, `SPEC.md:382`
...
**Files:** `alsc/references/compiler-error-shape.md:5,70`, `alsc/references/diagnostic-codes.md:37-86`, `alsc/references/module-skill-definition.md:63-86`, `alsc/references/manifest-template.md:3-53`
...
**Status:** unresolved
**Severity:** significant — blocks `als-mutate`/`als-migrate` implementation beyond file-presence checks

The spec clearly requires `vN+1/migrations/MANIFEST.md`, says `als-migrate` must fail if the manifest is missing or invalid, and requires a migration report with per-record status and failures. But the exact validation contract is incomplete.
...
**Files:** `SPEC.md:402-418`, `SPEC.md:558`, `SPEC.md:619`, `alsc/references/manifest-template.md:3-53`, `alsc/references/module-skill-definition.md:85`
...
`SPEC.md` Section 15 now limits baseline compiler scope to static schema/module validation. Workflow rules, transition legality, and transition guards are runtime concerns owned by module write behavior, not `alsc validate`.
...
**Files:** `alsc/references/record-validation.md:100-103`, `alsc/references/content-schema-definition.md:68-72`, `alsc/references/diagnostic-codes.md:68`
...
7. `### 5.` CLI interface for `alsc validate` is undesigned
